...
public class CareerServiceTest {
    ...
    public void testGetAllPaths_Success() {
        CareerPath path1 = CareerPath.builder().pathId(1).code("java-backend").name("Java Backend Engineer").build();
        CareerPath path2 = CareerPath.builder().pathId(2).code("frontend-engineer").name("Frontend Engineer").build();
    ...
    public void testGetPathNodes_Success() {
        Integer pathId = 1;
        CareerNode node1 = CareerNode.builder().nodeId(1L).pathId(pathId).name("Java Basics").level(1).build();
        CareerNode node2 = CareerNode.builder().nodeId(2L).pathId(pathId).name("Spring Boot").level(2).build();

        when(nodeRepository.findByPathIdOrderByLevelAsc(pathId)).thenReturn(Arrays.asList(node1, node2));
    ...
    @Test
    @DisplayName("Test Get User Progress - Query from DB")
    public void testGetUserProgress_Success() {
        Long userId = 1L;
        List<UserCareerProgress> dbProgress = List.of(
    ...
    public void testUnlockNode_Success() {
        ...
        UserCareerProgress progress = UserCareerProgress.builder()
                .userId(userId).nodeId(nodeId).status("LOCKED").build();

        when(progressRepository.findByUserIdAndNodeId(userId, nodeId)).thenReturn(Optional.of(progress));
        when(progressRepository.save(any(UserCareerProgress.class))).thenReturn(progress);

        careerService.unlockNode(userId, nodeId);

        verify(progressRepository, times(1)).save(argThat(p -> "UNLOCKED".equals(p.getStatus())));
    }
    ...
    public void testCompleteNode_Success() {
        ...
        Long nodeId = 10L;
        UserCareerProgress progress = UserCareerProgress.builder()
                .userId(userId).nodeId(nodeId).status("UNLOCKED").build();
        ...
        verify(progressRepository, times(1)).save(argThat(p -> "COMPLETED".equals(p.getStatus())));
    }
}
